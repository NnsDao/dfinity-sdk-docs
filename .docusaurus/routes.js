
import React from 'react';
import ComponentCreator from '@docusaurus/ComponentCreator';
export default [
{
  path: '/',
  component: ComponentCreator('/','deb'),
  exact: true,
},
{
  path: '/__docusaurus/debug',
  component: ComponentCreator('/__docusaurus/debug','3d6'),
  exact: true,
},
{
  path: '/__docusaurus/debug/config',
  component: ComponentCreator('/__docusaurus/debug/config','914'),
  exact: true,
},
{
  path: '/__docusaurus/debug/content',
  component: ComponentCreator('/__docusaurus/debug/content','c28'),
  exact: true,
},
{
  path: '/__docusaurus/debug/globalData',
  component: ComponentCreator('/__docusaurus/debug/globalData','3cf'),
  exact: true,
},
{
  path: '/__docusaurus/debug/metadata',
  component: ComponentCreator('/__docusaurus/debug/metadata','31b'),
  exact: true,
},
{
  path: '/__docusaurus/debug/registry',
  component: ComponentCreator('/__docusaurus/debug/registry','0da'),
  exact: true,
},
{
  path: '/__docusaurus/debug/routes',
  component: ComponentCreator('/__docusaurus/debug/routes','244'),
  exact: true,
},
{
  path: '/blog',
  component: ComponentCreator('/blog','ad6'),
  exact: true,
},
{
  path: '/blog/2021/06/08/verifying-icp-canister',
  component: ComponentCreator('/blog/2021/06/08/verifying-icp-canister','67d'),
  exact: true,
},
{
  path: '/blog/dfinance-airdrop-icp',
  component: ComponentCreator('/blog/dfinance-airdrop-icp','5cb'),
  exact: true,
},
{
  path: '/blog/dfinity-Canistore',
  component: ComponentCreator('/blog/dfinity-Canistore','f4b'),
  exact: true,
},
{
  path: '/blog/dfinity-SailFash',
  component: ComponentCreator('/blog/dfinity-SailFash','974'),
  exact: true,
},
{
  path: '/blog/dfinity-stacking-vote-reward',
  component: ComponentCreator('/blog/dfinity-stacking-vote-reward','9df'),
  exact: true,
},
{
  path: '/blog/icp-badlands-concept',
  component: ComponentCreator('/blog/icp-badlands-concept','488'),
  exact: true,
},
{
  path: '/blog/icp-basic-intro',
  component: ComponentCreator('/blog/icp-basic-intro','022'),
  exact: true,
},
{
  path: '/blog/icp-cycle-faucet',
  component: ComponentCreator('/blog/icp-cycle-faucet','776'),
  exact: true,
},
{
  path: '/blog/icp-dev-chat-fature',
  component: ComponentCreator('/blog/icp-dev-chat-fature','0a2'),
  exact: true,
},
{
  path: '/blog/icp-grants',
  component: ComponentCreator('/blog/icp-grants','ac3'),
  exact: true,
},
{
  path: '/blog/icp-intro',
  component: ComponentCreator('/blog/icp-intro','ab2'),
  exact: true,
},
{
  path: '/blog/icp-intro-Ecology',
  component: ComponentCreator('/blog/icp-intro-Ecology','cb0'),
  exact: true,
},
{
  path: '/blog/icp-nns-dfinity-next',
  component: ComponentCreator('/blog/icp-nns-dfinity-next','561'),
  exact: true,
},
{
  path: '/blog/icp-nns-dfinity-part',
  component: ComponentCreator('/blog/icp-nns-dfinity-part','70e'),
  exact: true,
},
{
  path: '/blog/icp-price-predictions',
  component: ComponentCreator('/blog/icp-price-predictions','95d'),
  exact: true,
},
{
  path: '/blog/icp-price-predictions-nnsdao',
  component: ComponentCreator('/blog/icp-price-predictions-nnsdao','232'),
  exact: true,
},
{
  path: '/blog/icp-price-predictions-nnsdao-25',
  component: ComponentCreator('/blog/icp-price-predictions-nnsdao-25','fec'),
  exact: true,
},
{
  path: '/blog/icp-stacking-guide',
  component: ComponentCreator('/blog/icp-stacking-guide','117'),
  exact: true,
},
{
  path: '/blog/icp-token-nns-dfinity',
  component: ComponentCreator('/blog/icp-token-nns-dfinity','301'),
  exact: true,
},
{
  path: '/blog/okex-icp-fund',
  component: ComponentCreator('/blog/okex-icp-fund','438'),
  exact: true,
},
{
  path: '/blog/page/2',
  component: ComponentCreator('/blog/page/2','0a4'),
  exact: true,
},
{
  path: '/blog/page/3',
  component: ComponentCreator('/blog/page/3','8bc'),
  exact: true,
},
{
  path: '/blog/tags',
  component: ComponentCreator('/blog/tags','fc1'),
  exact: true,
},
{
  path: '/blog/tags/airdrop',
  component: ComponentCreator('/blog/tags/airdrop','582'),
  exact: true,
},
{
  path: '/blog/tags/badlands',
  component: ComponentCreator('/blog/tags/badlands','6a6'),
  exact: true,
},
{
  path: '/blog/tags/cancan',
  component: ComponentCreator('/blog/tags/cancan','60c'),
  exact: true,
},
{
  path: '/blog/tags/candid',
  component: ComponentCreator('/blog/tags/candid','788'),
  exact: true,
},
{
  path: '/blog/tags/canistore',
  component: ComponentCreator('/blog/tags/canistore','91f'),
  exact: true,
},
{
  path: '/blog/tags/cycle',
  component: ComponentCreator('/blog/tags/cycle','44b'),
  exact: true,
},
{
  path: '/blog/tags/cycles',
  component: ComponentCreator('/blog/tags/cycles','5ef'),
  exact: true,
},
{
  path: '/blog/tags/dapp',
  component: ComponentCreator('/blog/tags/dapp','b42'),
  exact: true,
},
{
  path: '/blog/tags/dfinance',
  component: ComponentCreator('/blog/tags/dfinance','f95'),
  exact: true,
},
{
  path: '/blog/tags/dfinity',
  component: ComponentCreator('/blog/tags/dfinity','351'),
  exact: true,
},
{
  path: '/blog/tags/dscvr',
  component: ComponentCreator('/blog/tags/dscvr','074'),
  exact: true,
},
{
  path: '/blog/tags/faucet',
  component: ComponentCreator('/blog/tags/faucet','5a1'),
  exact: true,
},
{
  path: '/blog/tags/grants',
  component: ComponentCreator('/blog/tags/grants','61a'),
  exact: true,
},
{
  path: '/blog/tags/icp',
  component: ComponentCreator('/blog/tags/icp','f75'),
  exact: true,
},
{
  path: '/blog/tags/icpdrops',
  component: ComponentCreator('/blog/tags/icpdrops','8cb'),
  exact: true,
},
{
  path: '/blog/tags/icpscan',
  component: ComponentCreator('/blog/tags/icpscan','82a'),
  exact: true,
},
{
  path: '/blog/tags/icp币',
  component: ComponentCreator('/blog/tags/icp币','339'),
  exact: true,
},
{
  path: '/blog/tags/ndp',
  component: ComponentCreator('/blog/tags/ndp','e8a'),
  exact: true,
},
{
  path: '/blog/tags/ndt',
  component: ComponentCreator('/blog/tags/ndt','3aa'),
  exact: true,
},
{
  path: '/blog/tags/nns',
  component: ComponentCreator('/blog/tags/nns','870'),
  exact: true,
},
{
  path: '/blog/tags/nnsdao',
  component: ComponentCreator('/blog/tags/nnsdao','f7a'),
  exact: true,
},
{
  path: '/blog/tags/polychain',
  component: ComponentCreator('/blog/tags/polychain','596'),
  exact: true,
},
{
  path: '/blog/tags/sail-fash',
  component: ComponentCreator('/blog/tags/sail-fash','a8a'),
  exact: true,
},
{
  path: '/blog/tags/windows-hello',
  component: ComponentCreator('/blog/tags/windows-hello','93e'),
  exact: true,
},
{
  path: '/blog/windows-hello-ic',
  component: ComponentCreator('/blog/windows-hello-ic','48a'),
  exact: true,
},
{
  path: '/markdown-page',
  component: ComponentCreator('/markdown-page','be1'),
  exact: true,
},
{
  path: '/docs',
  component: ComponentCreator('/docs','d96'),
  
  routes: [
{
  path: '/docs/cli-reference/candid-types',
  component: ComponentCreator('/docs/cli-reference/candid-types','be1'),
  exact: true,
},
{
  path: '/docs/cli-reference/com-reference',
  component: ComponentCreator('/docs/cli-reference/com-reference','970'),
  exact: true,
},
{
  path: '/docs/cli-reference/dfx-envars',
  component: ComponentCreator('/docs/cli-reference/dfx-envars','25f'),
  exact: true,
},
{
  path: '/docs/cli-reference/glossary',
  component: ComponentCreator('/docs/cli-reference/glossary','1ba'),
  exact: true,
},
{
  path: '/docs/cli-reference/lang-service-ide',
  component: ComponentCreator('/docs/cli-reference/lang-service-ide','bbf'),
  exact: true,
},
{
  path: '/docs/developers-guide/at-a-glance',
  component: ComponentCreator('/docs/developers-guide/at-a-glance','347'),
  exact: true,
},
{
  path: '/docs/developers-guide/tutorials-intro',
  component: ComponentCreator('/docs/developers-guide/tutorials-intro','694'),
  exact: true,
},
{
  path: '/docs/Integration/auth-how-to-ic',
  component: ComponentCreator('/docs/Integration/auth-how-to-ic','37e'),
  exact: true,
},
{
  path: '/docs/Integration/ledger-quick-start',
  component: ComponentCreator('/docs/Integration/ledger-quick-start','d7f'),
  exact: true,
},
{
  path: '/docs/Integration/what-is-ic-identity',
  component: ComponentCreator('/docs/Integration/what-is-ic-identity','a3a'),
  exact: true,
},
{
  path: '/docs/intro',
  component: ComponentCreator('/docs/intro','e84'),
  exact: true,
},
{
  path: '/docs/language-guide/about-this-guide',
  component: ComponentCreator('/docs/language-guide/about-this-guide','ce5'),
  exact: true,
},
{
  path: '/docs/language-guide/basic-concepts',
  component: ComponentCreator('/docs/language-guide/basic-concepts','414'),
  exact: true,
},
{
  path: '/docs/language-guide/motoko',
  component: ComponentCreator('/docs/language-guide/motoko','7a6'),
  exact: true,
},
{
  path: '/docs/language-guide/motoko-introduction',
  component: ComponentCreator('/docs/language-guide/motoko-introduction','2c5'),
  exact: true,
},
{
  path: '/docs/language-guide/mutable-state',
  component: ComponentCreator('/docs/language-guide/mutable-state','f2d'),
  exact: true,
},
{
  path: '/docs/quick-start/dfinity-progarmming-practice',
  component: ComponentCreator('/docs/quick-start/dfinity-progarmming-practice','b75'),
  exact: true,
},
{
  path: '/docs/quick-start/dfx-guide',
  component: ComponentCreator('/docs/quick-start/dfx-guide','6ac'),
  exact: true,
},
{
  path: '/docs/quick-start/ic',
  component: ComponentCreator('/docs/quick-start/ic','17a'),
  exact: true,
},
{
  path: '/docs/quick-start/motoko',
  component: ComponentCreator('/docs/quick-start/motoko','f4e'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-book',
  component: ComponentCreator('/docs/rust-guide/rust-book','264'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-cargo-config',
  component: ComponentCreator('/docs/rust-guide/rust-cargo-config','aa6'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-cargo-config-detail',
  component: ComponentCreator('/docs/rust-guide/rust-cargo-config-detail','433'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-cargo-env',
  component: ComponentCreator('/docs/rust-guide/rust-cargo-env','725'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-cargo-shell',
  component: ComponentCreator('/docs/rust-guide/rust-cargo-shell','380'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-command-detail',
  component: ComponentCreator('/docs/rust-guide/rust-command-detail','de3'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-guide',
  component: ComponentCreator('/docs/rust-guide/rust-guide','21b'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-intro',
  component: ComponentCreator('/docs/rust-guide/rust-intro','3af'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-micro',
  component: ComponentCreator('/docs/rust-guide/rust-micro','9dd'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-programmer',
  component: ComponentCreator('/docs/rust-guide/rust-programmer','cca'),
  exact: true,
},
{
  path: '/docs/rust-guide/rust-unit-test',
  component: ComponentCreator('/docs/rust-guide/rust-unit-test','c40'),
  exact: true,
},
{
  path: '/docs/sample-apps/apps',
  component: ComponentCreator('/docs/sample-apps/apps','868'),
  exact: true,
},
{
  path: '/docs/sample-apps/some-dapp',
  component: ComponentCreator('/docs/sample-apps/some-dapp','7ec'),
  exact: true,
},
{
  path: '/docs/token-holders/custody-options-intro',
  component: ComponentCreator('/docs/token-holders/custody-options-intro','772'),
  exact: true,
},
{
  path: '/docs/token-holders/nns-app-quickstart',
  component: ComponentCreator('/docs/token-holders/nns-app-quickstart','ac7'),
  exact: true,
},
{
  path: '/docs/token-holders/self-custody-quickstart',
  component: ComponentCreator('/docs/token-holders/self-custody-quickstart','3c4'),
  exact: true,
},
{
  path: '/docs/tutorial-basics/create-a-document',
  component: ComponentCreator('/docs/tutorial-basics/create-a-document','f0d'),
  exact: true,
},
{
  path: '/docs/tutorial-basics/create-a-page',
  component: ComponentCreator('/docs/tutorial-basics/create-a-page','ca5'),
  exact: true,
},
{
  path: '/docs/tutorial-extras/canisters-code',
  component: ComponentCreator('/docs/tutorial-extras/canisters-code','4b0'),
  exact: true,
},
{
  path: '/docs/tutorial-extras/data-centers',
  component: ComponentCreator('/docs/tutorial-extras/data-centers','527'),
  exact: true,
},
{
  path: '/docs/tutorial-extras/governance',
  component: ComponentCreator('/docs/tutorial-extras/governance','9f7'),
  exact: true,
},
{
  path: '/docs/tutorial-extras/interface-spec',
  component: ComponentCreator('/docs/tutorial-extras/interface-spec','1f6'),
  exact: true,
},
{
  path: '/docs/tutorial-extras/manage-docs-versions',
  component: ComponentCreator('/docs/tutorial-extras/manage-docs-versions','d64'),
  exact: true,
},
{
  path: '/docs/tutorial-extras/tokens-cycles',
  component: ComponentCreator('/docs/tutorial-extras/tokens-cycles','881'),
  exact: true,
},
{
  path: '/docs/tutorial-extras/translate-your-site',
  component: ComponentCreator('/docs/tutorial-extras/translate-your-site','16a'),
  exact: true,
},
{
  path: '/docs/tutorial-how/customize-projects',
  component: ComponentCreator('/docs/tutorial-how/customize-projects','9a7'),
  exact: true,
},
{
  path: '/docs/tutorial-how/default-wallet',
  component: ComponentCreator('/docs/tutorial-how/default-wallet','c45'),
  exact: true,
},
{
  path: '/docs/tutorial-how/design-apps',
  component: ComponentCreator('/docs/tutorial-how/design-apps','945'),
  exact: true,
},
{
  path: '/docs/tutorial-how/install-upgrade-remove',
  component: ComponentCreator('/docs/tutorial-how/install-upgrade-remove','fe4'),
  exact: true,
},
{
  path: '/docs/tutorial-how/sdk-guide',
  component: ComponentCreator('/docs/tutorial-how/sdk-guide','2e7'),
  exact: true,
},
{
  path: '/docs/tutorial-how/troubleshooting',
  component: ComponentCreator('/docs/tutorial-how/troubleshooting','0b2'),
  exact: true,
},
{
  path: '/docs/tutorial-how/webpack-config',
  component: ComponentCreator('/docs/tutorial-how/webpack-config','596'),
  exact: true,
},
{
  path: '/docs/tutorial-how/work-with-languages',
  component: ComponentCreator('/docs/tutorial-how/work-with-languages','ac0'),
  exact: true,
},
{
  path: '/docs/tutorial-how/working-with-canisters',
  component: ComponentCreator('/docs/tutorial-how/working-with-canisters','1b3'),
  exact: true,
},
{
  path: '/docs/tutorial-wasms/wasm-intro',
  component: ComponentCreator('/docs/tutorial-wasms/wasm-intro','310'),
  exact: true,
},
]
},
{
  path: '*',
  component: ComponentCreator('*')
}
];
